...
describe('Special - shared target', () => {
  describe('when the same scope', () => {
    test('be the same', () => {
      ...
      container.provide({ token: A, useClass: TargetClass });
      container.provide({ token: B, useClass: TargetClass });
  ...
  describe('when binding Global -> Local', () => {
    ...
    test('throw', () => {
      ...
      container.provide({ token: A, useClass: TargetClass, scope: Scope.Global });
  ...
  describe('when binding Transient -> Local', () => {
    ...
    test('act as local', () => {
      ...
      container.provide({ token: A, useClass: TargetClass, scope: Scope.Transient });
      container.provide({ token: B, useClass: TargetClass, scope: Scope.Transient });
